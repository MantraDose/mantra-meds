...
5. **Call out user-supplement tasks:** Identify and list any steps only the user can do (e.g. create Vercel KV, set env vars, add secrets, configure cron, deploy, run manual checks). These will appear in a dedicated "Your tasks" (or equivalent) section.
6. **Insert a pause after each task group:** After every task group, add a clear "Pause for you" (or "Pause for review") block so the user can verify (e.g. run tests, manual check, test any UI) before continuing to the next group.
